...
export default Ember.Component.extend({
  ...
  didInsertElement() {
    ...
    (typeof FastBoot === 'undefined')) {
      ...
      this.set('map', new google.maps.Map(canvas, { draggable: false, scrollwheel: false } ));
    }
    this.setZoom();
    this.setCenter();
  ...
  placesChanged(place) {
    ...
    if(!searchedPlace) {
      ...
      if (places.length == 0) {
        ...
        searchedPlace = places[0]
      }
    }

    this.setMarkerAndBounds(searchedPlace);

    this._callCallback('placeChangedCallback', searchedPlace);
  },
